...
def main(yaml_name):
    ...
    #     # NOTE: The pre-simulation logic currently uses a single Cr_ref value.
    #     # If your Cr_ref is a list, we are using the first value for generating the "true" data.
    #     # This part could be enhanced later if needed.
    #     if isinstance(Cr_ref, list):
    #         Cr_ref = Cr_ref[0] # Use the first value for simulation
    #         print(f"Using first Cr_ref value for simulation: {Cr_ref}")
